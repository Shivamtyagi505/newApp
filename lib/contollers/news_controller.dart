...
class NewsController extends GetxController {
  ...
  void handleIndexChanged(int i) {
    pageController.animateToPage(i, duration: const Duration(microseconds: 200), curve: Curves.elasticInOut);
    selectedTab = i;
    update();
  }
  ...
  getAllNews({channel = '', searchKey = '', reload = false}) async {
    ...
    if (channel != '') {
      country.value = '';
      category.value = '';
      baseUrl = "https://newsapi.org/v2/top-headlines?sources=$channel&apiKey=${NewsApiConstants.newsApiKey}";
    }
    if (searchKey != '') {
      country.value = '';
      category.value = '';
      baseUrl = "https://newsapi.org/v2/everything?q=$searchKey&from=2022-07-01&sortBy=popularity&pageSize=10&apiKey=${NewsApiConstants.newsApiKey}";
    }
    // calling the API function and passing the URL here
  ...
  void updateValue(ArticleModel data, bool value) {
