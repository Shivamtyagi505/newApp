import 'package:news_app/constants/newsApi_constants.dart';
...
class NewsRepository {
  Future<List<NewsModel>> fetchNews() async {
    String url = 'https://newsapi.org/v2/top-headlines?q=business&languages=en&apiKey=c3c8e3a211ed419c8da525aedac6625f';
  ...
  Future<List<NewsModel>> fetchBreakingNews() async {
    String url = "https://newsapi.org/v2/top-headlines?country=in&apiKey=${NewsApiConstants.newsApiKey}";
    ...
    if (response.statusCode == 200) {
      for (var data in jsonData[NewsApiConstants.articles]) {
        if (data[NewsApiConstants.description].toString().isNotEmpty && data[NewsApiConstants.urlToImage].toString().isNotEmpty) {
          NewsModel articleModel = NewsModel.fromJson(data);
          articleModelList.add(articleModel);
        }
      }
  ...
  Future<List<NewsModel>> searchNews({required String query}) async {
    String url = '';
    if (query.isEmpty) {
      url = 'https://newsapi.org/v2/everything?q=biden&from=2022-07-12&sortBy=popularity&apiKey=c3c8e3a211ed419c8da525aedac6625f';
    } else {
